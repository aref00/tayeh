...
export class ConnectionOptionsReader {
    ...
    async read (): Promise<ConnectionOptions> {
        ...
        if (!options)
            throw new Error(`No connection options were found in any ty configuration files.`);
        return options;
    }

    plainRead (): ConnectionOptions {
        var config = JSON.parse(fs.readFileSync('/media/hdd/aref/Documents/projects/gitcha/mersilady/mercilady-web/tyconfig.json', 'utf8'));
